...
class MemoryBlock:
  ...
  def write(self, direct, value):
    real_addr = int(direct - self.start)
  ...
  def get_int_addr(self, addr): return int(addr)
  def get_float_addr(self, addr): return int(addr - self.seg_size)
  def get_char_addr(self, addr): return int(addr - self.seg_size * 2)
  def get_bool_addr(self, addr): return int(addr - self.seg_size * 3)
